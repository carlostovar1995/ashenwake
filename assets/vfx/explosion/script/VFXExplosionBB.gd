@tool
extends "res://assets/vfx/common/script/VFXControllerBB.gd"
...
@export var smoke_texture : Texture2D:
	set(v):
		...
		# Duplicating a NoiseTexture2D rebuilds it on the CPU. The packed scene
		# already has a normal map, so only do this in the editor.
		if Engine.is_editor_hint() and smoke_texture is NoiseTexture2D:
