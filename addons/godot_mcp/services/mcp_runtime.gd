...
func _handle_async_request(method: String, params: Dictionary) -> Dictionary:
	match method:
		"capture_screenshot":
			return await _capture_screenshot(params.get("path", "user://mcp_game_screenshot.png"))
		"capture_frames":
...
## Must run after a rendered frame — get_image() during _process often hangs
## or never writes an IPC response.
func _capture_screenshot(path: String) -> Dictionary:
	await get_tree().process_frame
	await get_tree().process_frame
	var viewport: Viewport = get_tree().root
	if viewport == null:
		return {"result": {"success": false, "error": "No root viewport"}}
	...
	if err != OK:
		return {"result": {"success": false, "error": "Failed to save screenshot: %s" % error_string(err)}}
	print("[MCP Runtime] Screenshot saved: %s (%dx%d)" % [save_path, image.get_width(), image.get_height()])
	return {"result": {"success": true, "path": save_path, "width": image.get_width(), "height": image.get_height()}}
