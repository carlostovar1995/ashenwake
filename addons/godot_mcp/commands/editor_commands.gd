...
func _get_game_screenshot(params: Dictionary) -> Dictionary:
	...
	# Do not overwrite an in-flight runtime request (scene tree + screenshot race).
	var idle_start: float = Time.get_unix_time_from_system()
	while FileAccess.file_exists(REQUEST_PATH) and Time.get_unix_time_from_system() - idle_start < IPC_TIMEOUT:
		if not _plugin.get_editor_interface().is_playing_scene():
			return {"success": false, "error": "Game stopped while waiting for IPC idle"}
		await _plugin.get_tree().process_frame
