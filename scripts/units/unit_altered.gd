...
const ICE_SPEED := 0.30


static func outgoing_damage_mult(u: Unit, ability_id: String, hit_kind: String) -> float:
	var m := 1.0
	if u.altered.shadow_stacks > 0 and u.altered.shadow_left > 0.05:
		m *= 1.0 + CombatBalance.pct("altered.shadow.out") * (float(u.altered.shadow_stacks) / float(SHADOW_MAX))
	if u.altered.fire_left > 0.05 and _is_spell_damage(ability_id, hit_kind):
...
static func resist_cut(u: Unit, hit_kind: String, ability_id: String) -> float:
	var cut := 0.0
	if u.altered.fire_left > 0.05 and (hit_kind.begins_with("fire") or hit_kind == "burn" or AbilityDef.matches_base(ability_id, "burn")):
		cut += CombatBalance.pct("altered.resist")
	if u.altered.ice_left > 0.05 and (hit_kind.begins_with("ice") or AbilityDef.matches_base(ability_id, "chilled_ground") or AbilityDef.matches_base(ability_id, "frost_trail")):
		cut += CombatBalance.pct("altered.resist")
	if u.altered.storm_left > 0.05 and (hit_kind.begins_with("lightning") or AbilityDef.matches_base(ability_id, "shock_chain")):
		cut += CombatBalance.pct("altered.resist")
	if u.altered.shadow_left > 0.05 and not AbilityDef.matches_base(ability_id, "shadow_pact"):
...
static func _apply_kind(u: Unit, kind: int, add_stack: bool) -> void:
	match kind:
		AbilityDef.Element.FIRE:
			u.altered.fire_left = BUFF_TIME
		AbilityDef.Element.ICE:
			u.altered.ice_left = BUFF_TIME
			if u.altered.frost_patches.is_empty():
				u.altered.ice_drop = u.global_position
				drop_frost(u)
		AbilityDef.Element.STORM:
			u.altered.storm_left = BUFF_TIME
		AbilityDef.Element.SHADOW:
			u.altered.shadow_left = BUFF_TIME
			if add_stack:
				u.altered.shadow_stacks = clampi(u.altered.shadow_stacks + 1, 1, SHADOW_MAX)
			elif u.altered.shadow_stacks < 1:
				u.altered.shadow_stacks = 1
...
static func tick(u: Unit, delta: float) -> void:
	if u.altered.fire_left > 0.0:
		u.altered.fire_left = maxf(0.0, u.altered.fire_left - delta)
	if u.altered.ice_left > 0.0:
		u.altered.ice_left = maxf(0.0, u.altered.ice_left - delta)
		if u.altered.ice_left > 0.0:
			tick_frost(u)
		else:
			clear_ice(u)
	else:
		if u.altered.on_frost_trail or not u.altered.frost_patches.is_empty():
			clear_ice(u)
	if u.altered.storm_left > 0.0:
		u.altered.storm_left = maxf(0.0, u.altered.storm_left - delta)
		if u.altered.storm_left > 0.0:
			u.altered.storm_acc += delta
			while u.altered.storm_acc >= STORM_TICK:
				u.altered.storm_acc -= STORM_TICK
				pulse_lightning(u)
		else:
			u.altered.storm_acc = 0.0
	if u.altered.shadow_left > 0.0:
		u.altered.shadow_left = maxf(0.0, u.altered.shadow_left - delta)
		if u.altered.shadow_left > 0.0:
			u.altered.shadow_acc += delta
			while u.altered.shadow_acc >= SHADOW_TICK and not u.is_dead:
				u.altered.shadow_acc -= SHADOW_TICK
				pulse_shadow(u)
		else:
			clear_shadow(u)
...
static func pulse_shadow(u: Unit) -> void:
	var ratio := float(clampi(u.altered.shadow_stacks, 1, SHADOW_MAX)) / float(SHADOW_MAX)
	...
	if u.altered.shadow_stacks < SHADOW_MAX:
		u.altered.shadow_stacks += 1
...
static func frost_def(_u: Unit) -> AbilityDef:
	if UnitAlteredState.frost_trail_ab == null:
		UnitAlteredState.frost_trail_ab = AbilityDef.make("frost_trail", "Frost Trail", "", AbilityDef.TargetMode.GROUND, 0.0, 0.0, 0.0, 0.0, Color(0.45, 0.82, 1.0))
		UnitAlteredState.frost_trail_ab.delivery = AbilityDef.Delivery.GROUND_AOE
		UnitAlteredState.frost_trail_ab.aoe_radius = ICE_RADIUS
		UnitAlteredState.frost_trail_ab.zone_duration = ICE_PATCH_TIME
		UnitAlteredState.frost_trail_ab.tick_interval = ICE_INTERVAL
		UnitAlteredState.frost_trail_ab.tick_damage = CombatBalance.scaled_hit("altered.ice.tick")
		UnitAlteredState.frost_trail_ab.element = AbilityDef.Element.ICE
		UnitAlteredState.frost_trail_ab.color = Color(0.45, 0.82, 1.0)
		UnitAlteredState.frost_trail_ab.vfx_primary = Color(0.55, 0.88, 1.0)
		UnitAlteredState.frost_trail_ab.vfx_secondary = Color(0.18, 0.45, 0.95)
	UnitAlteredState.frost_trail_ab.tick_damage = CombatBalance.scaled_hit("altered.ice.tick")
	return UnitAlteredState.frost_trail_ab


static func tick_frost(u: Unit) -> void:
	prune_frost(u)
	var moved := Vector2(u.global_position.x - u.altered.ice_drop.x, u.global_position.z - u.altered.ice_drop.z).length()
	if u.altered.frost_patches.is_empty() or moved >= ICE_DROP_DIST:
		drop_frost(u)
	u.altered.on_frost_trail = standing_on_frost(u)


static func drop_frost(u: Unit) -> void:
	var patch := _GroundAoe.spawn(u, u.global_position, frost_def(u), PackedInt32Array())
	u.altered.frost_patches.append(patch)
	u.altered.ice_drop = u.global_position


static func standing_on_frost(u: Unit) -> bool:
	for raw in u.altered.frost_patches:
...
static func prune_frost(u: Unit) -> void:
	...
	for raw in u.altered.frost_patches:
		if raw != null and is_instance_valid(raw):
			keep.append(raw)
	u.altered.frost_patches = keep


static func clear_ice(u: Unit) -> void:
	u.altered.ice_left = 0.0
	u.altered.on_frost_trail = false
	for raw in u.altered.frost_patches:
		if raw != null and is_instance_valid(raw):
			raw.queue_free()
	u.altered.frost_patches.clear()


static func clear_shadow(u: Unit) -> void:
	u.altered.shadow_left = 0.0
	u.altered.shadow_stacks = 0
	u.altered.shadow_acc = 0.0


static func clear_all(u: Unit) -> void:
	u.altered.fire_left = 0.0
	u.altered.storm_left = 0.0
	u.altered.storm_acc = 0.0
	clear_ice(u)
	clear_shadow(u)
...
static func _is_spell_damage(ability_id: String, hit_kind: String) -> bool:
	...
	if key == "shock_chain" or key == "frost_trail" or key == "shadow_pact" or key == "chilled_ground":
		return false
	if key == "seed_splash" or key == "seed_bloom" or key == "sunder_break" or key == "judge_mend":
		return false
